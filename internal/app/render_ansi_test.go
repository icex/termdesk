...
// ── remapPaletteColor tests (per-window theme override) ──

func TestRemapPaletteColor_NilPaletteReturnsSame(t *testing.T) {
	c := ansi.BasicColor(1)
	if got := remapPaletteColor(c, nil); got != c {
		t.Errorf("remapPaletteColor(basic, nil) = %v, want unchanged %v", got, c)
	}
}

func TestRemapPaletteColor_BasicColorMappedToOverride(t *testing.T) {
	var p [16]color.Color
	p[1] = color.RGBA{R: 0, G: 255, B: 0, A: 255} // override index 1 = pure green
	got := remapPaletteColor(ansi.BasicColor(1), &p)
	rgba, ok := got.(color.RGBA)
	if !ok {
		t.Fatalf("remapped color type = %T, want color.RGBA", got)
	}
	if rgba.R != 0 || rgba.G != 255 || rgba.B != 0 {
		t.Errorf("remapped rgba = %v, want {0 255 0 255}", rgba)
	}
}

func TestRemapPaletteColor_IndexedBelow16MappedToOverride(t *testing.T) {
	var p [16]color.Color
	p[9] = color.RGBA{R: 123, G: 45, B: 67, A: 255}
	got := remapPaletteColor(ansi.IndexedColor(9), &p)
	rgba, ok := got.(color.RGBA)
	if !ok {
		t.Fatalf("remapped color type = %T, want color.RGBA", got)
	}
	if rgba.R != 123 || rgba.G != 45 || rgba.B != 67 {
		t.Errorf("remapped rgba = %v, want {123 45 67 255}", rgba)
	}
}

func TestRemapPaletteColor_Indexed16OrAboveUntouched(t *testing.T) {
	var p [16]color.Color
	p[0] = color.RGBA{R: 1, G: 2, B: 3, A: 255} // should NOT apply to idx 42
	c := ansi.IndexedColor(42)
	if got := remapPaletteColor(c, &p); got != c {
		t.Errorf("remapPaletteColor(42) = %v, want unchanged %v", got, c)
	}
}

func TestRemapPaletteColor_TrueColorUntouched(t *testing.T) {
	var p [16]color.Color
	p[1] = color.RGBA{R: 0, G: 255, B: 0, A: 255}
	c := color.RGBA{R: 200, G: 100, B: 50, A: 255}
	if got := remapPaletteColor(c, &p); got != c {
		t.Errorf("remapPaletteColor(truecolor) = %v, want unchanged %v", got, c)
	}
}

func TestRemapPaletteColor_NilColorReturnsNil(t *testing.T) {
	var p [16]color.Color
	p[0] = color.RGBA{R: 1, G: 2, B: 3, A: 255}
	if got := remapPaletteColor(nil, &p); got != nil {
		t.Errorf("remapPaletteColor(nil) = %v, want nil", got)
	}
}

func TestRemapPaletteColor_NilPaletteSlotLeavesBasicUntouched(t *testing.T) {
	// If the override palette has a nil slot for that index, leave the
	// original palette color alone so the host terminal still renders it.
	var p [16]color.Color
	c := ansi.BasicColor(3)
	if got := remapPaletteColor(c, &p); got != c {
		t.Errorf("remapPaletteColor(basic, nil-slot) = %v, want unchanged %v", got, c)
	}
}

...
// contentThemeCache memoizes parsed override themes by name. Built-in themes
// never mutate, so the cache is safe to hold forever. Unused entries are cheap.
var contentThemeCache sync.Map // map[string]*config.Theme

// globalContentThemeName holds the user-configured default terminal content
// theme (UserConfig.TerminalTheme). Empty string means "follow the UI theme".
// Written only from the BT main goroutine via SetGlobalContentThemeName;
// read from render and Update paths on that same goroutine.
var globalContentThemeName string

// SetGlobalContentThemeName updates the package-global default content theme
// applied to windows that have no per-window ThemeOverride. Called at app
// init and whenever the user changes the "Terminal Theme" setting.
func SetGlobalContentThemeName(name string) {
	globalContentThemeName = name
}

// effectiveContentThemeName returns the theme name that should drive this
// window's content colors, or "" when the global UI theme should be used
// unchanged. Priority: per-window override > global TerminalTheme > UI theme.
func effectiveContentThemeName(w *window.Window) string {
	if w != nil && w.ThemeOverride != "" {
		return w.ThemeOverride
	}
	return globalContentThemeName
}

// contentThemeFor returns the theme whose DefaultFg/ContentBg/ANSIPalette
// should apply to this window's terminal content. Falls back to the global
// UI theme when no override applies. The returned theme always has
// ParseColors() called.
func contentThemeFor(w *window.Window, global config.Theme) config.Theme {
	name := effectiveContentThemeName(w)
	if name == "" {
		return global
	}
	if v, ok := contentThemeCache.Load(name); ok {
		return *(v.(*config.Theme))
	}
	t := config.GetTheme(name)
	t.ParseColors()
	contentThemeCache.Store(name, &t)
	return t
}

// paletteOverrideFor returns the 16-color palette to remap BasicColor/low
// IndexedColor cells through, or nil if the window inherits the global UI
// theme verbatim. When nil is returned, renderTerminalContent skips remapping
// and lets the host terminal apply its own palette.
func paletteOverrideFor(w *window.Window, global config.Theme) *[16]color.Color {
	// Theme selection (global Terminal Theme or per-window override) only
	// changes default fg/bg and window chrome — cells colored with ANSI
	// basics are left alone so the host terminal keeps painting them from
	// its own palette. Forcing the theme's pastel RGB palette over ANSI
	// cells makes content look faded vs. the host's native colors, which
	// is almost never what the user wants when picking a theme.
	return nil
}

// remapPaletteColor translates an ANSI 16-color palette index to the override
// palette's RGB value. Non-palette colors (nil, truecolor, indexed ≥ 16) are
// returned unchanged.
func remapPaletteColor(c color.Color, palette *[16]color.Color) color.Color {
	if c == nil || palette == nil {
		return c
	}
	...
	if mapped := palette[idx]; mapped != nil {
		return mapped
	}
	return c
}
...
func renderWindowTerminalContentWithSnapshot(buf *Buffer, w *window.Window, theme config.Theme, term *terminal.Terminal, showCursor bool, scrollOffset int, copySnap *CopySnapshot) {
	...
	// Content colors (default fg/bg + ANSI palette) may come from a per-window
	// theme override. Border/title bar/scroll indicator/cursor accent stay on
	// the global theme so the frame matches the rest of the desktop.
	contentTheme := contentThemeFor(w, theme)
	cc := contentTheme.C()
	contentBg := cc.ContentBg
	if contentBg == nil {
		contentBg = borderBg
	}
	defaultFg := cc.DefaultFg
	palette := paletteOverrideFor(w, theme)
	renderTerminalContentWithSnapshotPalette(buf, contentRect, term, defaultFg, contentBg, scrollOffset, copySnap, palette)
...
// renderTerminalContentWithSnapshotPalette renders terminal content and, when
// palette is non-nil, remaps any palette-indexed (BasicColor/IndexedColor 0–15)
// Fg/Bg values in the painted region to the palette's RGB. This is how a
// per-window theme override escapes the host terminal applying its own 16
// colors: by the time BufferToString runs, palette-indexed cells have become
// truecolor and are emitted as \e[38;2;R;G;Bm.
func renderTerminalContentWithSnapshotPalette(buf *Buffer, area geometry.Rect, term *terminal.Terminal, defaultFg, defaultBg color.Color, scrollOffset int, copySnap *CopySnapshot, palette *[16]color.Color) {
	renderTerminalContentWithSnapshot(buf, area, term, defaultFg, defaultBg, scrollOffset, copySnap)
	if palette == nil {
		return
	}
	...
	for y := area.Y; y < yEnd; y++ {
		...
		for x := area.X; x < xEnd; x++ {
			...
			cell := &row[x]
			cell.Fg = remapPaletteColor(cell.Fg, palette)
			cell.Bg = remapPaletteColor(cell.Bg, palette)
		}
	}
}

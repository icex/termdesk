...
func (m Model) handleKeyPress(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	if m.themePicker != nil {
		...
		case "enter":
			choice := ""
			if m.themePicker.Index >= 0 && m.themePicker.Index < len(m.themePicker.Choices) {
				choice = m.themePicker.Choices[m.themePicker.Index]
			}
			m.setWindowTheme(m.themePicker.WindowID, choice)
			m.themePicker = nil
		case "up", "k":
		...
		case "down", "j":
			if m.themePicker.Index < len(m.themePicker.Choices)-1 {
				m.themePicker.Index++
			}
		case "home":
			m.themePicker.Index = 0
		case "end":
			m.themePicker.Index = len(m.themePicker.Choices) - 1
		}
		return m, nil
	}

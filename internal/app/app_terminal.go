...
type TerminalWindowOpts struct {
	...
	ThemeOverride string   // optional per-window theme (content only); empty = inherit global
}

// windowContentDefaults returns the default fg/bg used for the emulator of
// terminals inside this window. Resolution follows contentThemeFor:
// per-window ThemeOverride > global TerminalTheme > global UI theme.
func (m *Model) windowContentDefaults(w *window.Window) (fg, bg color.Color) {
	ct := contentThemeFor(w, m.theme)
	c := ct.C()
	return c.DefaultFg, c.ContentBg
}

// applyWindowContentTheme re-applies emulator default colors for a window's
// main terminal and any split-pane terminals, using the window's current
// theme (override if set, else the global theme). Called after the window's
// ThemeOverride changes so programs that later clear the screen pick up the
// new defaults.
func (m *Model) applyWindowContentTheme(w *window.Window) {
	if w == nil {
		return
	}
	fg, bg := m.windowContentDefaults(w)
	if term, ok := m.terminals[w.ID]; ok && term != nil {
		term.SetDefaultColors(fg, bg)
	}
	if w.IsSplit() && w.SplitRoot != nil {
		for _, termID := range w.SplitRoot.AllTermIDs() {
			if term, ok := m.terminals[termID]; ok && term != nil {
				term.SetDefaultColors(fg, bg)
			}
		}
	}
...
// setWindowTheme updates a window's content theme override and refreshes the
// emulator defaults for every terminal inside that window. Passing "" clears
// the override so the window inherits the global theme again.
func (m *Model) setWindowTheme(windowID, themeName string) {
	w := m.wm.WindowByID(windowID)
	if w == nil {
		return
	}
	w.ThemeOverride = themeName
	m.applyWindowContentTheme(w)
}

// openThemePickerForFocused opens the per-window theme picker for the
// currently-focused terminal window. No-op when no terminal window is focused.
func (m *Model) openThemePickerForFocused() {
	fw := m.wm.FocusedWindow()
	if fw == nil {
		return
	}
	hasTerm := m.terminals[fw.ID] != nil || (fw.IsSplit() && fw.SplitRoot != nil)
	if !hasTerm {
		return
	}
	choices := append([]string{""}, config.ThemeNames()...)
	idx := 0
	for i, name := range choices {
		if name == fw.ThemeOverride {
			idx = i
			break
		}
	}
	m.themePicker = &ThemePickerDialog{
		WindowID: fw.ID,
		Choices:  choices,
		Index:    idx,
	}
...
func (m *Model) createTerminalWindow(opts TerminalWindowOpts) tea.Cmd {
	...
	win.ThemeOverride = opts.ThemeOverride
	if opts.Maximized {
	...
	// When the window has a ThemeOverride, use its content colors instead.
	tfg, tbg := m.windowContentDefaults(win)
	term.SetDefaultColors(tfg, tbg)
...
func (m *Model) spawnPTYReader(windowID string, term *terminal.Terminal) {
	...
	term.SetOnOutput(func() {
		...
		if now.Sub(lastSend) >= minInterval {
			...
			lastSend = now
			go safeSend(p, PtyOutputMsg{WindowID: windowID})
		} else if pending == nil {
			pending = time.AfterFunc(minInterval, func() {
				...
				lastSend = time.Now()
				mu.Unlock()
				// Close() races with a fired-but-not-yet-run AfterFunc:
				// .Stop() returned false but the callback body is already
				// scheduled. Skip the send if the terminal is torn down —
				// p.Send on a quit program panics in BT v2.
				if term.IsClosed() {
					return
				}
				go safeSend(p, PtyOutputMsg{WindowID: windowID})
		...
		term.SetOnKittyGraphics(func(cmd *terminal.KittyCommand, rawData []byte) *terminal.PlacementResult {
			...
			if cmd.Action == terminal.KittyActionQuery {
				...
				if out := kp.FlushPending(); len(out) > 0 {
					...
					go safeSend(p, KittyFlushMsg{Data: out})
				}
				return nil
			}
		...
		term.SetOnImageGraphics(func(rawData []byte, format terminal.ImageFormat, estimatedCellRows int) int {
			...
			if estimatedCellRows == 0 {
				...
				if out := ip.FlushPending(); len(out) > 0 {
					go safeSend(p, ImageFlushMsg{Data: out})
				}
				return 0
			}
	...
	// Wire screen clear callback — clears graphics when child app sends CSI 2J/3J.
	{
		...
		term.SetOnScreenClear(func() {
			...
			if ip != nil && ip.ClearWindow(windowID) {
				...
				go safeSend(p, ImageClearScreenMsg{})
			}
		})
	}
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				...
				safeSend(p, PtyClosedMsg{WindowID: windowID, Err: fmt.Errorf("internal panic: %v", r)})
			}
		}()
		...
		for {
			...
			if err != nil {
				...
				mu.Unlock()
				// Dispatch notification + close in goroutines so we don't
				// stall the reader exit on BT's unbuffered message channel
				// (the same pattern used everywhere else in this file).
				go safeSend(p, PtyOutputMsg{WindowID: windowID})
				go safeSend(p, PtyClosedMsg{WindowID: windowID, Err: err})
				return
			}
		}
	}()
...
// safeSend forwards msg to the tea.Program, swallowing the panic that
// occurs when the program has already returned from Run() (BT v2 sends
// on a closed internal channel). Used on shutdown paths where callbacks
// may fire between closeAllTerminals and the BT main loop exiting.
...
func (m *Model) toggleQuakeTerminal() tea.Cmd {
	...
	// Resize terminal to current screen size. Skip if dimensions are
	// unchanged — emu.Resize clears the screen even on a no-op resize, which
	// would wipe the shell's prompt on every reopen.
	cols, rows := m.quakeContentSize()
	if m.quakeTerminal.Width() != cols || m.quakeTerminal.Height() != rows {
...
// isPlacementStale inspects the cells under a Kitty placement and returns
// true if virtually all of them contain non-space content — a strong
// signal that an image previewer switched to text without emitting an
// explicit Kitty delete (yazi → text preview is the canonical case).
//
// Conservative on purpose — false positives evict legitimate images and
// flash the user. Specifically:
//   - Hidden (not yet shown) placements are never stale.
//   - Placements shown less than 1.5s ago are skipped — tools like yazi
//     write their preview-pane chrome (borders, file name, etc.) right
//     after the image, and those cells must settle before we can draw
//     conclusions about whether they were "overwritten with text".
//   - Placements whose origin is in scrollback are left alone.
//   - A placement is stale only when ≥90% of its footprint has non-space
//     content AND the footprint is at least a few cells; a single stray
//     status glyph must not evict an image.
func (m *Model) isPlacementStale(winID string, p *terminal.KittyPlacement) bool {
	if p == nil || p.Hidden {
		return false
	}
	if p.DisplayRows <= 0 || p.Cols <= 0 {
		return false
	}
	if !p.ShownAt.IsZero() && time.Since(p.ShownAt) < 1500*time.Millisecond {
		return false
	}
	// Tiny placements (1–2 cells) don't give us enough signal — skip.
	if p.DisplayRows*p.Cols < 4 {
		return false
	}
	term := m.terminals[winID]
	if term == nil {
		// Split-pane case: geometry is stored under the window ID but
		// the terminal owning the placement is a pane. Try any pane
		// terminal in the window — every pane shares the emulator grid.
		if w := m.wm.WindowByID(winID); w != nil && w.IsSplit() && w.SplitRoot != nil {
			for _, paneID := range w.SplitRoot.AllTermIDs() {
				if t := m.terminals[paneID]; t != nil {
					term = t
					break
				}
			}
	...
	startRow := p.AbsoluteLine - scrollback
	if startRow < 0 {
		// Image anchor has scrolled into scrollback; we can't read
		// those cells reliably, so leave the placement alone.
		return false
	}
	overwritten := 0
	total := 0
	for dy := 0; dy < p.DisplayRows; dy++ {
		for dx := 0; dx < p.Cols; dx++ {
			c := term.CellAt(p.GuestX+dx, startRow+dy)
			if c == nil {
				continue
			}
			total++
			// Empty and space-only cells are what tools reserve for
			// images — anything else means something wrote real text
			// over that cell.
			s := c.Content
			if s == "" || s == " " {
				continue
	...
	// Require ≥50% of cells to have been overwritten to consider stale.
	// Tools often draw borders/labels around image previews — those
	// contribute a minority of non-space cells. A majority of text
	// under the image is a strong signal it should be evicted.
	stale := total >= 4 && overwritten*2 >= total
	kittyDbg("isPlacementStale: winID=%s hostID=%d guestX=%d absLine=%d rows=%d cols=%d startRow=%d scrollback=%d total=%d overwritten=%d stale=%v shownAgo=%v virtual=%v",
		winID, p.HostImageID, p.GuestX, p.AbsoluteLine, p.DisplayRows, p.Cols,
		startRow, scrollback, total, overwritten, stale,
		time.Since(p.ShownAt), p.Virtual)
	return stale
}

// pruneStaleKittyOnly runs the staleness heuristic on cached Kitty
// placements and queues delete APCs for any the heuristic evicts. Does
// NOT re-emit surviving placements — safe to call on every PtyOutputMsg
// without causing the "re-emit every frame → paint-then-overwrite"
// flicker that full refreshKittyPlacements would produce.
//
// IMPORTANT: only calls FlushPending when something was actually
// evicted. Otherwise we'd drain bytes the passthrough callback queued
// for transmission (e.g. in-flight SHOW commands meant to trigger
// KittyFlushMsg → ImageRefreshMsg → RefreshAllPlacements), stranding
// freshly-placed images in the Hidden state forever.
func (m *Model) pruneStaleKittyOnly() {
	if m.kittyPass == nil || !m.kittyPass.HasPlacements() {
		return
	}
	if m.hasImageBlockingOverlay() || m.imageState.hidden {
		return
	}
	evicted := m.kittyPass.PruneStalePlacements(func(winID string, p *terminal.KittyPlacement) bool {
		return m.isPlacementStale(winID, p)
	})
	if evicted == 0 {
		return
	}
...
func (m *Model) refreshKittyPlacements() {
	...
	// callback (running in emuWriteLoop goroutine) has fresh data. Split
	// panes register their callback keyed by pane ID, so seed each pane
	// under the owning window's geometry too.
	for _, w := range allWindows {
		updateKittyGeom(w.ID, w)
		if w.IsSplit() && w.SplitRoot != nil {
			for _, paneID := range w.SplitRoot.AllTermIDs() {
				updateKittyGeom(paneID, w)
			}
		}
	}

	// Prune placements whose underlying terminal cells have been overwritten
	// with non-space text. This catches tools like yazi that switch from
	// image preview to text preview without emitting an explicit Kitty
	// delete — otherwise the stale image keeps re-appearing on every refresh.
	m.kittyPass.PruneStalePlacements(func(winID string, p *terminal.KittyPlacement) bool {
		return m.isPlacementStale(winID, p)
	})

...
func (m *Model) splitPane(dir window.SplitDir) tea.Cmd {
	...
	fg, bg := m.windowContentDefaults(fw)
	term.SetDefaultColors(fg, bg)
	m.terminals[newTermID] = term
	m.termCreatedAt[newTermID] = time.Now()
	// The kitty callback uses whatever ID was passed to spawnPTYReader as its
	// geometry key. For split panes that's the pane ID, so we have to seed
	// the window's geometry under the pane ID too — otherwise the callback
	// bails at kittyWindowGeom.Load and images silently never render.
	updateKittyGeom(newTermID, fw)

...
const (
	...
	DragResizeSE
	DragResizeSW
	DragSeparator // dragging a split pane separator
)
...
type DragState struct {
	...
	// Separator drag fields (only valid when Mode == DragSeparator)
	SepNode *SplitNode // the internal node owning this separator
	SepDir  SplitDir   // direction of the separator being dragged
}
...
func HitTest(w *Window, p geometry.Point, closeButtonWidth, maxButtonWidth int) HitZone {
	...
	if localY < tbh {
		...
		if localY == btnRow || tbh == 1 {
			// When maximized, buttons extend to the edge (no border inset).
			rightEdge := lastCol
			if !maximized {
				rightEdge = lastCol
			}
			// Buttons right-to-left: [snapL][snapR][max][_][×]
			closeStart := rightEdge - closeButtonWidth
			if localX > closeStart && localX <= rightEdge {
		...
		// Top-left / top-right corners on first row = resize zones (not when maximized)
		if !maximized && localY == 0 {
			if localX == 0 {
				return HitBorderNW
			}
			if localX == lastCol {
	...
	// Maximized: no side/bottom borders — everything below title bar is content
	if maximized {
		return HitContent
	}

...
// RenderThemePickerDialog draws a vertical list picker for choosing the
// per-window terminal content theme. Mirrors the lipgloss style used by the
// other dialogs. The currently-selected row is rendered with the accent
// color; all other rows use the dialog's content background. A short
// footer hint ("Enter select · Esc cancel") sits below the list.
func RenderThemePickerDialog(buf *Buffer, dialog *ThemePickerDialog, theme config.Theme) {
	...
	// Inner width: fits the longest choice label plus padding; enforce a
	// minimum so the dialog doesn't look cramped.
	maxW := runeLen(title)
	for _, name := range dialog.Choices {
		label := themePickerLabel(name)
		if w := runeLen(label); w > maxW {
			maxW = w
		}
	}
	innerW := maxW + 4
	...
	rows := make([]string, 0, len(dialog.Choices))
	for i, name := range dialog.Choices {
		label := themePickerLabel(name)
		if i == dialog.Index {
			rows = append(rows, selRowStyle.Render(label))
		} else {
	...
	footerStyle := lipgloss.NewStyle().
		...
		Width(innerW).
		Align(lipgloss.Center)
	footerStr := footerStyle.Render("Enter select · Esc cancel")

	parts := []string{titleStr, sepStr}
	parts = append(parts, rows...)
	parts = append(parts, sepStr, footerStr)
...
// themePickerLabel renders a choice name for display. Empty string becomes
// "(Inherit global)" so the user can see what the first row does.
func themePickerLabel(name string) string {
	if name == "" {

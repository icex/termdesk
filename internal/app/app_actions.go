...
func (m Model) confirmAccept() (tea.Model, tea.Cmd) {
	...
	if m.confirmClose.IsQuit {
		// Three-phase quit so the BT event loop stays responsive:
		// 1. signal apps now (fast), dismiss dialog
		// 2. tea.Tick 150ms → QuitSaveMsg (gives vim/apps time to respond)
		// 3. QuitSaveMsg → async SaveWorkspace → QuitFinalizeMsg → close + quit
		projectDir := ""
		...
		return m, tea.Tick(150*time.Millisecond, func(time.Time) tea.Msg {
			return QuitSaveMsg{ProjectDir: projectDir}
		})
	}
...
func (m Model) executeMenuAction(action string) (tea.Model, tea.Cmd) {
	...
	case "new_workspace":
		...
		m.newWorkspaceDialog = makeNewWorkspaceDialog(cwd)
		return m, nil
	case "window_theme_picker":
		m.openThemePickerForFocused()
		return m, nil
	case "save_workspace":
		m.saveWorkspaceNow()
		return m, nil
...
func (m *Model) applySettings() {
	...
	cfg.TerminalTheme = globalContentThemeName
	cfg.IconsOnly = m.dock.IconsOnly
	cfg.DockPill = m.dock.Pill
	...
	prevTermThemeName := globalContentThemeName
	themeChanged := cfg.Theme != m.theme.Name
	termThemeChanged := cfg.TerminalTheme != prevTermThemeName
	if themeChanged {
		m.theme = config.GetTheme(cfg.Theme)
	}
	if termThemeChanged {
		SetGlobalContentThemeName(cfg.TerminalTheme)
	}
	// Re-apply emulator defaults whenever the effective content theme may
	// have shifted for non-overridden windows so programs that later clear
	// the screen pick up the new default fg/bg. Windows with a per-window
	// ThemeOverride are pinned and left alone.
	if themeChanged || termThemeChanged {
		for _, w := range m.wm.Windows() {
			if w.ThemeOverride != "" {
				continue
			}

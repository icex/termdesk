...
// When isTerminal is true, a "Terminal Theme…" item is appended for opening
// the per-window content-theme picker.
func TitleBarMenu(x, y int, resizable, isSplit bool, kb KeyBindings, isTerminal bool) *Menu {
	items := []Item{
	...
	if isTerminal {
		items = append(items,
			Item{Label: "─", Disabled: true},
			Item{Label: "\uf1fc  Terminal Theme…", Action: "window_theme_picker"},
		)
	}
	return &Menu{
		X: x, Y: y,

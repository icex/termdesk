...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if hasKitty || hasImages {
		...
		default:
			...
			case PtyOutputMsg:
				// Terminal output can't move windows, so skip full
				// refresh (re-emitting images every frame would paint
				// then overwrite — flicker). But output CAN overwrite
				// cells under a stale placement (yazi switching from
				// image to text preview), so run the cheap prune-only
				// path: cell scan + optional delete emit, no re-place.
				if hasKitty {
					ret.pruneStaleKittyOnly()
				}
			case tea.RawMsg, CursorBlinkMsg, SystemStatsMsg, CleanupMsg,
				...
				// image-relevant content churn. Skipping these prevents
				// flicker. KittyFlushMsg is intentionally NOT here — new
				// placements arrive Hidden=true and need a refresh cycle
				// to emit the SHOW command.
			default:
...
func (m Model) handleUpdate(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case QuitSaveMsg:
		...
		return m, func() tea.Msg {
			if err := workspace.SaveWorkspace(state, pd); err != nil {
				logging.Error("workspace save failed: %v", err)
			}
			return QuitFinalizeMsg{}
		}

	case QuitFinalizeMsg:
		m.closeAllTerminals()
		return m, tea.Quit

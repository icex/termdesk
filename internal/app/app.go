...
// ThemePickerDialog represents a list picker for choosing a window's per-
// terminal content theme. Index 0 is always the "inherit global" row.
type ThemePickerDialog struct {
	WindowID string
	Choices  []string // [""] + config.ThemeNames(); "" means inherit
	Index    int
}

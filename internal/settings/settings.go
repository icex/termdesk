...
// terminalThemeInheritLabel is the sentinel value shown in the choice list
// when the user wants the terminal content to follow the global UI theme.
// Stored in UserConfig.TerminalTheme as "" — translated on both ends.
// Lowercase, no parens — matches the visual weight of the real theme names
// ("modern", "sleek", …) so the cell doesn't read as faded next to them.
const terminalThemeInheritLabel = "inherit"

// terminalThemeChoices returns the choices list for the "Terminal Theme"
// setting: the inherit sentinel followed by every registered theme.
func terminalThemeChoices() []string {
	names := config.ThemeNames()
	out := make([]string, 0, len(names)+1)
	out = append(out, terminalThemeInheritLabel)
	out = append(out, names...)
	return out
}

// terminalThemeChoice returns the display value for the current config value.
func terminalThemeChoice(stored string) string {
	if stored == "" {
		return terminalThemeInheritLabel
	}
	return stored
}

// TerminalThemeFromChoice converts a choice-list StrVal back to the stored
// UserConfig.TerminalTheme value (empty string for inherit).
func TerminalThemeFromChoice(choice string) string {
	if choice == terminalThemeInheritLabel {
		return ""
	}
	return choice
}
...
func New(cfg config.UserConfig, wMetas []widget.WidgetMeta, wEnabled []string) *Panel {
	general := Section{
		...
		Items: []Item{
			...
			{Key: "terminal_theme", Label: "Terminal Theme", Type: TypeChoice, StrVal: terminalThemeChoice(cfg.TerminalTheme), Choices: terminalThemeChoices()},
	...
	dockSection := Section{
		...
		Items: []Item{
			...
			{Key: "dock_pill", Label: "Pill Shape (transparent sides)", Type: TypeToggle, BoolVal: cfg.DockPill},
		},
	}
...
func (p *Panel) ApplyTo(cfg *config.UserConfig) {
	...
	for _, sec := range p.Sections {
		for _, item := range sec.Items {
			...
			case "terminal_theme":
				cfg.TerminalTheme = TerminalThemeFromChoice(item.StrVal)
			case "animations":

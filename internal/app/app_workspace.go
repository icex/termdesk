...
func (m *Model) restoreWorkspace(state *workspace.WorkspaceState, projectDir string) {
	...
	for _, ws := range state.Windows {
		...
		win.ThemeOverride = ws.ThemeOverride
		...
		m.wm.AddWindow(win)
		// Seed kitty geometry cache so the onKittyGraphics callback can look
		// up this window as soon as PTY output starts arriving. Without this,
		// image tools (kitten icat / chafa --format=kitty) run inside a
		// restored window fire their APC callback but bail out at
		// kittyWindowGeom.Load — so bytes are intercepted but never forwarded
		// to the host terminal and the cursor moves down into blank space.
		updateKittyGeom(win.ID, win)
		...
		if ws.SplitTree != "" && len(ws.Panes) > 0 {
			...
			for _, pr := range panes {
				...
				pfg, pbg := m.windowContentDefaults(win)
				paneTerm.SetDefaultColors(pfg, pbg)
				m.terminals[pr.TermID] = paneTerm
				m.termCreatedAt[pr.TermID] = time.Now()
				// Kitty callback uses the pane ID as its geometry key.
				updateKittyGeom(pr.TermID, win)
			...
			if err == nil && term != nil {
				wfg, wbg := m.windowContentDefaults(win)
				term.SetDefaultColors(wfg, wbg)

...
type KittyPlacement struct {
	...
	// ShownAt is the monotonic time when the placement was last
	// transitioned from hidden→visible. Used by heuristics that must
	// avoid evicting an image immediately after it was painted (zero
	// value means "never shown yet").
	ShownAt time.Time
}
...
// HostTermProgram returns the real host terminal's identity, or "".
// Used to propagate TERM_PROGRAM to child PTY processes so image tools
// (kitten, chafa, …) see the actual host terminal (e.g. "ghostty") instead
// of a spoofed value.
//
// In session mode the session server strips TERM_PROGRAM before spawning
// the --app child (to keep BT v2 from enabling features behind a PTY proxy)
// and preserves the original under TERMDESK_HOST_TERM_PROGRAM. Prefer the
// shadow var; fall back to TERM_PROGRAM for direct (non-session) runs.
func HostTermProgram() string {
	if v := os.Getenv("TERMDESK_HOST_TERM_PROGRAM"); v != "" {
		return v
	}
...
func (kp *KittyPassthrough) forwardDelete(cmd *KittyCommand, windowID string) {
	kittyPassDbg("forwardDelete: windowID=%s delete=%c(%d) imageID=%d placementID=%d placementsBefore=%d",
		windowID, byte(cmd.Delete), int(cmd.Delete), cmd.ImageID, cmd.PlacementID, len(kp.placements[windowID]))
	switch cmd.Delete {
	...
	case KittyDeleteByID:
		if windowMap := kp.imageIDMap[windowID]; windowMap != nil {
			if hostID, ok := windowMap[cmd.ImageID]; ok {
				...
				kittyPassDbg("forwardDelete: KittyDeleteByID imageID=%d not found in windowMap for %s (have %d entries)",
					cmd.ImageID, windowID, len(windowMap))
			}
		} else {
			kittyPassDbg("forwardDelete: KittyDeleteByID but no windowMap for %s", windowID)
		}
...
// PruneStalePlacements walks all stored placements and calls isStale for
// each. When isStale returns true the placement's image is evicted from
// the cache AND a delete APC is queued to pending output. Returns the
// number of placements that were evicted so callers can decide whether
// FlushPending is worth calling.
//
// Used to clean up images left behind by tools (like yazi) that switch
// from image preview to text preview without issuing an explicit Kitty
// delete (a=d).
//
// isStale is called with the lock held, so it must not call back into
// this passthrough. Only call it while holding the BT main goroutine.
func (kp *KittyPassthrough) PruneStalePlacements(isStale func(windowID string, p *KittyPlacement) bool) int {
	kp.mu.Lock()
	defer kp.mu.Unlock()
	evicted := 0
	for winID, placements := range kp.placements {
		for hostID, p := range placements {
			if !isStale(winID, p) {
				continue
			}
			...
			// Scrub guest→host mapping so later deletes from the
			// child app can still resolve correctly for other images.
			if m := kp.imageIDMap[winID]; m != nil {
				for guestID, hID := range m {
					if hID == hostID {
						delete(m, guestID)
						break
					}

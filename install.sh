...
install_apps() {
    ...
    local apps=("htop" "mc" "nvim" "spf" "carbonyl")
    local missing=()
    ...
    for app in "${missing[@]}"; do
        case "${OS}:${app}" in
            ...
            termux:mc)        pkgs+=("mc");;
            termux:spf)       ;; # install via go install
            termux:carbonyl)  ;; # manual install from GitHub releases
            macos:nvim)       pkgs+=("neovim");;
            macos:htop)       pkgs+=("htop");;
            macos:mc)         pkgs+=("mc");;
            macos:spf)        pkgs+=("superfile");;
            macos:carbonyl)   ;; # manual install from GitHub releases
            linux:nvim)       pkgs+=("neovim");;
            linux:htop)       pkgs+=("htop");;
            linux:mc)         pkgs+=("mc");;
            linux:spf)        ;; # install via go install
            linux:carbonyl)   ;; # manual install from GitHub releases
        esac
    done
...
build() {
    ...
    # Build targets: main binary + bundled apps
    local targets=(
        "bin/termdesk:./cmd/termdesk"
    )

    # Discover apps in apps/*/main.go
    for app_dir in "${SCRIPT_DIR}"/apps/*/; do
        if [[ -f "${app_dir}main.go" ]]; then
            local app_name
            app_name="termdesk-$(basename "${app_dir}")"
            targets+=("bin/${app_name}:${app_dir}")
        fi
    done
    ...
    for target in "${targets[@]}"; do
        ...
        if [[ "${OS}" == "termux" ]]; then
            # Termux runs on Android but Go detects GOOS=linux. Android's Bionic
            ...
            info "Building ${out} (GOOS=android CGO_ENABLED=0)"
            CGO_ENABLED=0 GOOS=android "${GO_CMD}" build -o "${out}" "${src}"
        else
            info "Building ${out}"
...
install_binary() {
    ...
    # Install all binaries from bin/
    for bin_file in "${SCRIPT_DIR}"/bin/termdesk*; do
        if [[ -x "${bin_file}" ]]; then
            local bin_name
    ...
    # Create 'td' symlink for quick access
    ln -sf "${install_dir}/termdesk" "${install_dir}/td"
    ok "Created symlink td -> termdesk"

    # Install app manifest files (.toml) next to binaries
    for app_dir in "${SCRIPT_DIR}"/apps/*/; do
        for toml_file in "${app_dir}"*.toml; do
            if [[ -f "${toml_file}" ]]; then
                local toml_name
                toml_name="$(basename "${toml_file}")"
                cp "${toml_file}" "${install_dir}/${toml_name}"
                ok "Installed manifest ${toml_name}"
            fi
        done
    done

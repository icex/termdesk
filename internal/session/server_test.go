...
func TestBuildAppEnv(t *testing.T) {
	...
	// The raw TERM_PROGRAM key is stripped — but its value is preserved
	// under TERMDESK_HOST_TERM_PROGRAM so child tools can still see the
	// real host terminal identity. Match as a line, not a substring, so
	// the shadow var (which contains "TERM_PROGRAM=" inside its key) does
	// not accidentally satisfy this check.
	for _, e := range env {
		if strings.HasPrefix(e, "TERM_PROGRAM=") {
			t.Fatalf("TERM_PROGRAM should be stripped, got %q", e)
		}
	}
	if !strings.Contains(joined, "TERMDESK_HOST_TERM_PROGRAM=SomeTerm") {
		t.Fatalf("TERMDESK_HOST_TERM_PROGRAM should preserve the host identity, got:\n%s", joined)
	}

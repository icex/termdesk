...
func renderSplitWindow(buf *Buffer, w *window.Window, theme config.Theme,
	...
	// Content colors (default fg/bg + palette) may come from a per-window
	// theme override. Border/separator colors stay on the global theme.
	...
	for _, p := range panes {
		...
		renderTerminalContentWithSnapshotPalette(buf, p.Rect, term, defaultFg, contentBg, paneScroll, copySnap, palette)

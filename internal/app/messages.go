...
// QuitSaveMsg is phase 2 of the quit flow: fires ~150ms after the user
// confirms quit so that app-state signals (vim :mksession, SIGUSR1) have
// time to land before we snapshot the workspace.
type QuitSaveMsg struct {
	ProjectDir string
}

// QuitFinalizeMsg is phase 3 of the quit flow: fires after the async
// SaveWorkspace goroutine completes, so the Update goroutine only calls
// closeAllTerminals + tea.Quit without blocking on disk I/O.
type QuitFinalizeMsg struct{}

...
func TestGeneralSectionItems(t *testing.T) {
	...
	if termTheme.Key != "terminal_theme" {
		t.Errorf("expected index 1 key 'terminal_theme', got %q", termTheme.Key)
	}
	if termTheme.Type != TypeChoice {
		t.Errorf("terminal_theme should be TypeChoice, got %d", termTheme.Type)
	}
	// Animations should be a toggle (index 2 after terminal_theme)
	anim := gen.Items[2]
...
func TestToggle(t *testing.T) {
	cfg := config.DefaultUserConfig()
	p := New(cfg, nil, nil)
	// Navigate to animations (index 2; terminal_theme sits between theme and animations)
	p.ActiveItem = 2
...
func TestCycleChoiceOnToggleDoesNothing(t *testing.T) {
	cfg := config.DefaultUserConfig()
	p := New(cfg, nil, nil)
	p.ActiveItem = 2 // animations (toggle)
...
func TestApplyTo(t *testing.T) {
	...
	// Toggle animations (General[2] after terminal_theme at [1])
	p.ActiveItem = 2
	p.Toggle()

	// Change animation speed (General[3])
	p.ActiveItem = 3
	p.CycleChoice(1)
	newSpeed := p.CurrentItem().StrVal

	// Change animation style (General[4])
	p.ActiveItem = 4
	p.CycleChoice(1)
...
func TestApplyToShowDeskClock(t *testing.T) {
	...
	// show_desk_clock is at index 5 in General section
	p.ActiveTab = 0
	p.ActiveItem = 5
...
func TestApplyToShowKeys(t *testing.T) {
	...
	// show_keys is at index 6 in General section
	p.ActiveTab = 0
	p.ActiveItem = 6
...
func TestApplyToTilingMode(t *testing.T) {
	...
	// tiling_mode is at index 7 in General section
	p.ActiveTab = 0
	p.ActiveItem = 7
...
func TestDockSectionStructure(t *testing.T) {
	...
	if len(dock.Items) != 5 {
		t.Fatalf("expected 5 items in Dock, got %d", len(dock.Items))
	}
	expectedKeys := []string{"icons_only", "minimize_to_dock", "hide_dock_when_maximized", "hide_dock_apps", "dock_pill"}
...
func TestApplyToDefaultTerminalMode(t *testing.T) {
	...
	// default_terminal_mode is at index 8 in General section
	p.ActiveTab = 0
	p.ActiveItem = 8

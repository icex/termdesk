...
func TestSettingsClickOnItem(t *testing.T) {
	...
	// Item at index 2 in General tab is "animations" (TypeToggle);
	// terminal_theme sits at index 1 between theme and animations.
	origAnimations := m.settings.Sections[0].Items[2].BoolVal
	...
	// Items start at relY=6, so item 2 (animations) is at relY=8
	itemY := startY + 1 + 8
	...
	newAnimations := m.settings.Sections[0].Items[2].BoolVal

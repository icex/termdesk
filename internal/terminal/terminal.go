...
type Terminal struct {
	...
	syncFireTimer  *time.Timer // deferred onOutput fire if no data arrives before the 100ms sync timeout
...
func (t *Terminal) emuWriteLoop() {
	...
	process:
		...
		if !syncing {
			...
			// Schedule a deferred fire so a shell that opens a sync block and
			// stays idle (zsh+p10k on first prompt — writes ESC[?2026h but the
			// closing ESC[?2026l rides a later chunk) doesn't leave the UI
			// blank until the user types. One pending timer at a time.
			t.mu.Lock()
			if t.syncFireTimer != nil {
				t.syncFireTimer.Stop()
			}
			t.syncFireTimer = time.AfterFunc(120*time.Millisecond, func() {
				t.mu.Lock()
				if t.closed {
					t.mu.Unlock()
					return
...
func (t *Terminal) Close() error {
	...
	pty = t.pty
	emu = t.emu
	// Nil out app-facing callbacks before the drain. emuWriteLoop may still
	// be mid-iteration and would otherwise re-enter onOutput/onBell/onImage/
	// onKittyGraphics/onScreenClear AFTER the Model has been torn down
	// (e.g. during tea.Quit). These callbacks typically do `go p.Send(...)`
	// or `p.Send(...)` on *tea.Program, which panics if the program has

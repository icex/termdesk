...
// disconnectClient closes the currently-connected client and clears s.client.
// Callers that hold a reference to a specific conn (e.g. handleClient) should
// use disconnectIfCurrent to avoid closing a newer client that replaced theirs.
...
// disconnectIfCurrent closes conn only if it is still the active client.
// This prevents an old handleClient goroutine (whose conn was replaced by
// acceptLoop) from closing the new client's connection when it finally
// notices its own conn was closed and tries to clean up.
func (s *Server) disconnectIfCurrent(conn net.Conn) {
	s.clientMu.Lock()
	defer s.clientMu.Unlock()
	if s.client == conn {
		s.client.Close()
	...
	// Not the active client — ensure our own conn is closed without
	// touching s.client (which is someone else's newer conn).
	if conn != nil {
		conn.Close()
	}
}
...
func BuildAppEnv() []string {
	...
	skip := map[string]bool{
		...
		"TERMDESK_HOST_TERM_PROGRAM": true,
	}
	var env []string
	...
	// Preserve the real host TERM_PROGRAM so the --app child (and grandchild
	// terminals it spawns) can propagate the actual terminal identity to image
	// tools (kitten, chafa) instead of spoofing a generic "WezTerm" fallback
	// that some tools special-case in unhelpful ways (e.g. chafa shrinking
	// images to 1 cell). We strip TERM_PROGRAM itself above to avoid BT v2
	// feature conflicts, and read back this shadow var in HostTermProgram.
	if hp := os.Getenv("TERM_PROGRAM"); hp != "" {
		env = append(env, "TERMDESK_HOST_TERM_PROGRAM="+hp)
	}
	if hasGraphics {

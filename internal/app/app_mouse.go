...
func (m Model) handleRightClick(mouse tea.Mouse) (tea.Model, tea.Cmd) {
	...
	for i := len(wins) - 1; i >= 0; i-- {
		...
		if hit == window.HitTitleBar {
			m.wm.FocusWindow(w.ID)
			isTerm := m.terminals[w.ID] != nil || (w.IsSplit() && w.SplitRoot != nil)
			m.contextMenu = contextmenu.TitleBarMenu(mouse.X, mouse.Y, w.Resizable, w.IsSplit(), ctxMenuKB(m.keybindings), isTerm)
			m.contextMenu.Clamp(m.width, m.height)
			return m, nil
		}

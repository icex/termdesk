...
func TestWindowStateThemeOverrideRoundTrip(t *testing.T) {
	state := WorkspaceState{
		Version: 1,
		SavedAt: time.Now(),
		Windows: []WindowState{
	...
	if !strings.Contains(toml, `theme_override = "redmond"`) {
		t.Errorf("serialized TOML missing theme_override, got:\n%s", toml)
	}
	...
	if got := parsed.Windows[0].ThemeOverride; got != "redmond" {
		t.Errorf("theme_override: got %q, want %q", got, "redmond")
	}
}

func TestWindowStateEmptyThemeOverrideOmitted(t *testing.T) {
	// Inherit-global (empty) should NOT be serialized — matches the
	// existing convention for optional string fields.
	state := WorkspaceState{
		Version: 1,
		SavedAt: time.Now(),
		Windows: []WindowState{
			{ID: "w", Title: "T", Command: "/bin/sh", X: 0, Y: 0, Width: 80, Height: 24},
		},
	}
	toml := serializeWorkspace(state)
	if strings.Contains(toml, "theme_override") {
		t.Errorf("empty theme_override should not be serialized, got:\n%s", toml)
	}
}
